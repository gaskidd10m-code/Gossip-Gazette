...
interface SportsNews { id: string; title: string; content: string; imageUrl?: string; category: 'Transfer News' | 'Sports Today' | 'General'; status: 'draft' | 'published'; createdAt: string; }
...
const db = {
    ...
    // Sports News
    async getSportsNews(): Promise<SportsNews[]> { const result = await executeSql(`SELECT * FROM sports_news ORDER BY created_at DESC`); return result.map((r: any) => ({ id: r.id, title: r.title, content: r.content, category: r.category as 'Transfer News' | 'Sports Today' | 'General', status: r.status as 'draft' | 'published', createdAt: r.created_at, imageUrl: r.image_url })); },
    ...
    async createSportsNews(data: Omit<SportsNews, 'id' | 'createdAt'> & { imageUrl?: string }): Promise<SportsNews> { const result = await executeSql(`INSERT INTO sports_news (title, content, category, status, image_url) VALUES ($1, $2, $3, $4, $5) RETURNING *`, [data.title, data.content, data.category || 'Sports Today', data.status || 'draft', data.imageUrl || null]); const r = result[0]; return { id: r.id, title: r.title, content: r.content, category: r.category, status: r.status, createdAt: r.created_at, imageUrl: r.image_url }; },
    async updateSportsNews(id: string, data: Partial<Omit<SportsNews, 'id' | 'createdAt'> & { imageUrl?: string }>): Promise<void> { const fields: string[] = []; const values: any[] = []; let idx = 1; if (data.title !== undefined) { fields.push(`title = $${idx++}`); values.push(data.title); } if (data.content !== undefined) { fields.push(`content = $${idx++}`); values.push(data.content); } if (data.category !== undefined) { fields.push(`category = $${idx++}`); values.push(data.category); } if (data.status !== undefined) { fields.push(`status = $${idx++}`); values.push(data.status); } if (data.imageUrl !== undefined) { fields.push(`image_url = $${idx++}`); values.push(data.imageUrl); } if (fields.length === 0) return; values.push(id); await executeSql(`UPDATE sports_news SET ${fields.join(', ')} WHERE id = $${idx}`, values); },
